...
torch.save({'model_state': model.state_dict()}, 'model_sod.pth')
###################################
model.eval()
...
savemat("pred_sod.mat", mdict={'pred':pred, 'trueX':x_test, 'trueY':y_test})
...
for i in range(3):
    ...
    plt.savefig("{}_pred_sod_{}.pdf".format(index, i+1))
    plt.close()