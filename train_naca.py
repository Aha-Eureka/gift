...
def load_data(path, ntrain, ntest):
    coords     = np.load(path + "supplementary_data/shape_coords.npy").astype("float32")#(N,120,2)
  
    vertices_x = np.load(path + "NACA/NACA_Cylinder_X.npy")[...,np.newaxis]
    vertices_y = np.load(path + "NACA/NACA_Cylinder_Y.npy")[...,np.newaxis]
    X          = np.concatenate((vertices_x, vertices_y), -1).astype("float32")
    Y          = np.load(path + "NACA/NACA_Cylinder_Q.npy")[:,:4,...].transpose(0,2,3,1).astype("float32")
